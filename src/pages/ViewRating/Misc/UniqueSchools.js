
const tureUniqueSchools = [
    {
        id: 'SANJOSESTATEUNIVERSITY',
        schoolName: 'San Jose State University'
      },
      {
        id: 'UNIVERSITYOFCALIFORNIARIVERSIDE',
        schoolName: 'University of California Riverside '
      },
      { id: 'MISSIONCOLLEGE', schoolName: 'Mission College' },
      {
        id: 'COLORADOSTATEUNIVERSITY',
        schoolName: 'Colorado State University'
      },
      {
        id: 'UNIVERSITYOFSOUTHFLORIDA',
        schoolName: 'University of South Florida'
      },
      {
        id: 'UNIVERSITYOFCALIFORNIADAVIS',
        schoolName: 'University of California Davis'
      },
      {
        id: 'UNIVERISTYOFCALIFRONIASANTACRUZ',
        schoolName: 'univeristy of califronia santa cruz'
      },
      { id: 'UNIVERSITYOFMICHIGAN', schoolName: 'University of Michigan ' },
      { id: 'STANFORDUNIVERSITY', schoolName: 'Stanford University' },
      {
        id: 'UNIVERSITYOFHOUSTONMAINCAMPUS',
        schoolName: 'University of Houston - Main Campus'
      },
      {
        id: 'NORTHWESTERNUNIVERSITY',
        schoolName: 'Northwestern University'
      },
      { id: 'UNIVERSITYOFTORONTO', schoolName: 'University of Toronto' },
      {
        id: 'GRANDVALLEYSTATEUNIVERSITY',
        schoolName: 'Grand Valley State University '
      },
      {
        id: 'RUTGERSUNIVERSITYNEWARK',
        schoolName: 'Rutgers University Newark'
      },
      {
        id: 'NORMANDALECOMMUNITYCOLLEGE',
        schoolName: 'Normandale community college '
      },
      {
        id: 'NORTHEASTERNUNIVERSITY',
        schoolName: 'Northeastern University'
      },
      {
        id: 'NEWJERSEYINSTITUTEOFTECHNOLOGY',
        schoolName: 'New Jersey Institute of Technology'
      },
      {
        id: 'UNIVERSITYOFILLINOISATCHICAGO',
        schoolName: 'University of Illinois at Chicago '
      },
      {
        id: 'UNIVERSITYOFWASHINGTON',
        schoolName: 'University of Washington'
      },
      {
        id: 'UNIVERSITYOFMARYLANDCOLLEGEPARK',
        schoolName: 'University of Maryland, College Park'
      },
      {
        id: 'LOYOLAUNIVERSITYCHICAGO',
        schoolName: 'Loyola University Chicago'
      },
      { id: 'DENISONUNIVERSITY', schoolName: 'Denison University ' },
      {
        id: 'GEORGIAINSTITUTEOFTECHNOLOGY',
        schoolName: 'Georgia institute of technology'
      },
      {
        id: 'UNIVERSITYOFTEXASATDALLAS',
        schoolName: 'University of Texas at Dallas'
      },
      {
        id: 'NORTHERNVIRGINIACOMMUNITYCOLLEGE',
        schoolName: 'Northern Virginia Community College'
      },
      { id: 'UNIVERSITYOFMIAMI', schoolName: 'University of Miami' },
      { id: 'UNIVERSITYOFWINDSOR', schoolName: 'University of Windsor' },
      {
        id: 'RUTGERSUNIVERSITYNEWBRUNSWICK',
        schoolName: 'Rutgers University - New Brunswick'
      },
      {
        id: 'UNIVERSITYOFTEXASSOUTHWESTERNMEDICALCENTER',
        schoolName: 'University of Texas Southwestern Medical Center'
      },
      {
        id: 'HAASSCHOOLOFBUSINESS',
        schoolName: 'Haas school of business '
      },
      {
        id: 'THEGEORGEWASHINGTONUNIVERSITY',
        schoolName: 'The George Washington University'
      },
      { id: 'BARUCHCOLLEGE', schoolName: 'Baruch college' },
      {
        id: 'UNIVERSITYOFCALIFORNIABERKELEY',
        schoolName: 'University of California Berkeley'
      },
      { id: 'HUNTERCOLLEGE', schoolName: 'Hunter College' },
      {
        id: 'YORKUNIVERSITY(TORONTOCANADA)',
        schoolName: 'York University (Toronto, Canada)'
      },
      { id: 'DEPAULUNIVERSITY', schoolName: 'DePaul University ' },
      { id: 'UNIVERSITYOFFLORIDA', schoolName: 'University of Florida' },
      {
        id: 'NATIONALUNIVERSITYOFSINGAPORE',
        schoolName: 'National University of Singapore'
      },
      { id: 'BROOKLYNCOLLEGE', schoolName: 'Brooklyn College' },
      {
        id: 'UNIVERSITYCOLLEGELONDON',
        schoolName: 'University College London'
      },
      {
        id: 'UNIVERSITYATALBANYSUNY',
        schoolName: 'University at Albany, SUNY'
      },
      { id: 'SANTACLARAUNIVERSITY', schoolName: 'Santa Clara University ' },
      {
        id: 'UNIVERSITYOFCINCINNATI',
        schoolName: 'University of Cincinnati '
      },
      { id: 'TEANECKHIGHSCHOOL', schoolName: 'Teaneck High School' },
      { id: 'RUTGERSUNIVERSITY', schoolName: 'Rutgers University' },
      {
        id: 'UNIVERSITYOFWESTERNONTARIO',
        schoolName: 'University of Western Ontario'
      },
      {
        id: 'ARIZONASTATEUNIVERSITY',
        schoolName: 'Arizona State University '
      },
      { id: 'UNIVERSITYOFMARYLAND', schoolName: 'University of Maryland' },
      {
        id: 'UNIVERSITYOFTEXASATAUSTIN',
        schoolName: 'University of Texas at Austin'
      },
      { id: 'AUBURNUNIVERSITY', schoolName: 'Auburn University ' },
      {
        id: 'UNIVERSITYOFMISSOURIKANSASCITY',
        schoolName: 'university of missouri-kansas city '
      },
      {
        id: 'UNIVERSITYOFCALIFORNIALOSANGELES',
        schoolName: 'University of California Los Angeles'
      },
      {
        id: 'THEUNIVERSITYOFTEXASATDALLAS',
        schoolName: 'The University of Texas at Dallas'
      },
      {
        id: 'UNIVERSITYOFTHEINCARNATEWORD',
        schoolName: 'University of the Incarnate Word '
      },
      {
        id: 'GEORGEMASONUNIVERSITY',
        schoolName: 'George Mason University '
      },
      { id: 'UNIVERSITYOFHOUSTON', schoolName: 'University of Houston ' },
      {
        id: 'UNIVERSITYOFPENNSYLVANIA',
        schoolName: 'University of Pennsylvania '
      },
      {
        id: 'UNIVERSITYOFMANCHESTER',
        schoolName: 'University of Manchester'
      },
      { id: 'ROWANUNIVERSITY', schoolName: 'Rowan University ' },
      { id: 'DREXELUNIVERSITY', schoolName: 'Drexel University' },
      {
        id: 'THELONDONSCHOOLOFECONOMICSANDPOLITICALSCIENCE',
        schoolName: 'The London school of economics and political science'
      },
      { id: 'ASTONUNIVERSITY', schoolName: 'Aston University' },
      { id: 'RYERSONUNIVERSITY', schoolName: 'Ryerson University' },
      { id: 'BINGHAMTONUNIVERSITY', schoolName: 'Binghamton University ' },
      {
        id: 'NORTHCAROLINASTATEUNIVERSITY',
        schoolName: 'North Carolina State University'
      },
      {
        id: 'NORTHERNALBERTAINSTITUTEOFTECHNOLOGY(NAIT)',
        schoolName: 'Northern Alberta Institute of Technology (NAIT)'
      },
      {
        id: 'THEUNIVERISTYOFTEXASATDALLAS',
        schoolName: 'The Univeristy of Texas at Dallas'
      },
      { id: 'UNIVERSITYOFWYOMING', schoolName: 'University of Wyoming' },
      {
        id: 'CALSTATESANBERNARDINO',
        schoolName: 'Cal state San Bernardino '
      },
      { id: 'ADELPHIUNIVERSITY', schoolName: 'Adelphi University ' },
      {
        id: 'UNIVERSITYOFCALIFORNIAIRVINE',
        schoolName: 'University of California, Irvine'
      },
      {
        id: 'UNIVERSITYOFMINNESOTATWINCITIES',
        schoolName: 'University of Minnesota - Twin Cities'
      },
      {
        id: 'CITYOFNEWYORKHUNTERCOLLEGE',
        schoolName: 'City of New York- Hunter College'
      },
      {
        id: 'FLORIDAINTERNATIONALUNIVERSITY',
        schoolName: 'Florida International University'
      },
      { id: 'DUKE', schoolName: 'Duke' },
      {
        id: 'UNIVERSITYOFMICHIGANANNARBOR',
        schoolName: 'University of Michigan Ann Arbor'
      },
      {
        id: 'CALIFORNIASTATEPOLYTECHNICUNIVERSITYPOMONA',
        schoolName: 'California state polytechnic university, Pomona'
      },
      { id: 'PURDUEUNIVERSITY', schoolName: 'Purdue University' },
      {
        id: 'WESTVALLEYCOMMUNITYCOLLEGE',
        schoolName: 'West Valley Community College'
      },
      {
        id: 'HERIOTWATTUNIVERSITY(EDINBURGH)',
        schoolName: 'Heriot-Watt University (Edinburgh)'
      },
      {
        id: 'UNIVERSITYOFWASHINGTONBOTHELL',
        schoolName: 'University of Washington Bothell'
      },
      {
        id: 'CENTRALMICHIGANUNIVERSITY',
        schoolName: 'Central Michigan University '
      },
      { id: 'BROWARDCOLLEGE', schoolName: 'Broward College' }
];

// const uniqueSchoolsObject = [
//     {
//       id: 'SANJOSESTATEUNIVERSITY',
//       schoolName: 'San Jose State University'
//     },
//     {
//       id: 'UNIVERSITYOFCALIFORNIARIVERSIDE',
//       schoolName: 'University of California Riverside '
//     },
//     { id: 'MISSIONCOLLEGE', schoolName: 'Mission College' },
//     {
//       id: 'COLORADOSTATEUNIVERSITY',
//       schoolName: 'Colorado State University'
//     },
//     {
//       id: 'UNIVERSITYOFSOUTHFLORIDA',
//       schoolName: 'University of South Florida'
//     },
//     {
//       id: 'UNIVERSITYOFCALIFORNIADAVIS',
//       schoolName: 'University of California Davis'
//     },
//     {
//       id: 'UNIVERISTYOFCALIFRONIASANTACRUZ',
//       schoolName: 'univeristy of califronia santa cruz'
//     },
//     { id: 'UNIVERSITYOFMICHIGAN', schoolName: 'University of Michigan ' },
//     { id: 'STANFORDUNIVERSITY', schoolName: 'Stanford University' },
//     {
//       id: 'UNIVERSITYOFHOUSTONMAINCAMPUS',
//       schoolName: 'University of Houston - Main Campus'
//     },
//     {
//       id: 'NORTHWESTERNUNIVERSITY',
//       schoolName: 'Northwestern University'
//     },
//     { id: 'UNIVERSITYOFTORONTO', schoolName: 'University of Toronto' },
//     {
//       id: 'GRANDVALLEYSTATEUNIVERSITY',
//       schoolName: 'Grand Valley State University '
//     },
//     {
//       id: 'RUTGERSUNIVERSITYNEWARK',
//       schoolName: 'Rutgers University Newark'
//     },
//     {
//       id: 'NORMANDALECOMMUNITYCOLLEGE',
//       schoolName: 'Normandale community college '
//     },
//     {
//       id: 'NORTHEASTERNUNIVERSITY',
//       schoolName: 'Northeastern University'
//     },
//     {
//       id: 'NEWJERSEYINSTITUTEOFTECHNOLOGY',
//       schoolName: 'New Jersey Institute of Technology'
//     },
//     {
//       id: 'UNIVERSITYOFILLINOISATCHICAGO',
//       schoolName: 'University of Illinois at Chicago '
//     },
//     {
//       id: 'UNIVERSITYOFWASHINGTON',
//       schoolName: 'University of Washington'
//     },
//     {
//       id: 'UNIVERSITYOFMARYLANDCOLLEGEPARK',
//       schoolName: 'University of Maryland, College Park'
//     },
//     {
//       id: 'UNIVERSITYOFCALIFORNIARIVERSIDE',
//       schoolName: 'University of California, Riverside'
//     },
//     {
//       id: 'LOYOLAUNIVERSITYCHICAGO',
//       schoolName: 'Loyola University Chicago'
//     },
//     { id: 'DENISONUNIVERSITY', schoolName: 'Denison University ' },
//     {
//       id: 'GEORGIAINSTITUTEOFTECHNOLOGY',
//       schoolName: 'Georgia institute of technology'
//     },
//     {
//       id: 'UNIVERSITYOFTEXASATDALLAS',
//       schoolName: 'University of Texas at Dallas'
//     },
//     {
//       id: 'NORTHERNVIRGINIACOMMUNITYCOLLEGE',
//       schoolName: 'Northern Virginia Community College'
//     },
//     { id: 'UNIVERSITYOFMIAMI', schoolName: 'University of Miami' },
//     { id: 'UNIVERSITYOFWINDSOR', schoolName: 'University of Windsor' },
//     {
//       id: 'RUTGERSUNIVERSITYNEWBRUNSWICK',
//       schoolName: 'Rutgers University - New Brunswick'
//     },
//     {
//       id: 'UNIVERSITYOFTEXASSOUTHWESTERNMEDICALCENTER',
//       schoolName: 'University of Texas Southwestern Medical Center'
//     },
//     {
//       id: 'HAASSCHOOLOFBUSINESS',
//       schoolName: 'Haas school of business '
//     },
//     {
//       id: 'THEGEORGEWASHINGTONUNIVERSITY',
//       schoolName: 'The George Washington University'
//     },
//     { id: 'BARUCHCOLLEGE', schoolName: 'Baruch college' },
//     {
//       id: 'UNIVERSITYOFCALIFORNIABERKELEY',
//       schoolName: 'University of California Berkeley'
//     },
//     { id: 'HUNTERCOLLEGE', schoolName: 'Hunter College' },
//     {
//       id: 'YORKUNIVERSITY(TORONTOCANADA)',
//       schoolName: 'York University (Toronto, Canada)'
//     },
//     { id: 'DEPAULUNIVERSITY', schoolName: 'DePaul University ' },
//     { id: 'UNIVERSITYOFFLORIDA', schoolName: 'University of Florida' },
//     {
//       id: 'NATIONALUNIVERSITYOFSINGAPORE',
//       schoolName: 'National University of Singapore'
//     },
//     { id: 'BROOKLYNCOLLEGE', schoolName: 'Brooklyn College' },
//     {
//       id: 'UNIVERSITYCOLLEGELONDON',
//       schoolName: 'University College London'
//     },
//     {
//       id: 'UNIVERSITYATALBANYSUNY',
//       schoolName: 'University at Albany, SUNY'
//     },
//     { id: 'SANTACLARAUNIVERSITY', schoolName: 'Santa Clara University ' },
//     {
//       id: 'UNIVERSITYOFCINCINNATI',
//       schoolName: 'University of Cincinnati '
//     },
//     { id: 'TEANECKHIGHSCHOOL', schoolName: 'Teaneck High School' },
//     { id: 'RUTGERSUNIVERSITY', schoolName: 'Rutgers University' },
//     {
//       id: 'UNIVERSITYOFWESTERNONTARIO',
//       schoolName: 'University of Western Ontario'
//     },
//     {
//       id: 'ARIZONASTATEUNIVERSITY',
//       schoolName: 'Arizona State University '
//     },
//     { id: 'UNIVERSITYOFMARYLAND', schoolName: 'University of Maryland' },
//     {
//       id: 'UNIVERSITYOFTEXASATAUSTIN',
//       schoolName: 'University of Texas at Austin'
//     },
//     {
//       id: 'GEORGIAINSTITUTEOFTECHNOLOGY',
//       schoolName: 'Georgia Institute of Technology'
//     },
//     { id: 'AUBURNUNIVERSITY', schoolName: 'Auburn University ' },
//     {
//       id: 'UNIVERSITYOFMISSOURIKANSASCITY',
//       schoolName: 'university of missouri-kansas city '
//     },
//     {
//       id: 'UNIVERSITYOFCALIFORNIALOSANGELES',
//       schoolName: 'University of California Los Angeles'
//     },
//     {
//       id: 'THEUNIVERSITYOFTEXASATDALLAS',
//       schoolName: 'The University of Texas at Dallas'
//     },
//     {
//       id: 'UNIVERSITYOFTHEINCARNATEWORD',
//       schoolName: 'University of the Incarnate Word '
//     },
//     {
//       id: 'GEORGEMASONUNIVERSITY',
//       schoolName: 'George Mason University '
//     },
//     { id: 'UNIVERSITYOFHOUSTON', schoolName: 'University of Houston ' },
//     {
//       id: 'UNIVERSITYOFCALIFORNIABERKELEY',
//       schoolName: 'University of California Berkeley'
//     },
//     {
//       id: 'UNIVERSITYOFPENNSYLVANIA',
//       schoolName: 'University of Pennsylvania '
//     },
//     {
//       id: 'UNIVERSITYOFMANCHESTER',
//       schoolName: 'University of Manchester'
//     },
//     {
//       id: 'UNIVERSITYOFILLINOISATCHICAGO',
//       schoolName: 'University of Illinois at Chicago'
//     },
//     { id: 'UNIVERSITYOFHOUSTON', schoolName: 'University of Houston' },
//     { id: 'ROWANUNIVERSITY', schoolName: 'Rowan University ' },
//     { id: 'DREXELUNIVERSITY', schoolName: 'Drexel University' },
//     {
//       id: 'THELONDONSCHOOLOFECONOMICSANDPOLITICALSCIENCE',
//       schoolName: 'The London school of economics and political science'
//     },
//     { id: 'ASTONUNIVERSITY', schoolName: 'Aston University' },
//     { id: 'RYERSONUNIVERSITY', schoolName: 'Ryerson University' },
//     { id: 'BINGHAMTONUNIVERSITY', schoolName: 'Binghamton University ' },
//     {
//       id: 'NORTHCAROLINASTATEUNIVERSITY',
//       schoolName: 'North Carolina State University'
//     },
//     {
//       id: 'NORTHERNALBERTAINSTITUTEOFTECHNOLOGY(NAIT)',
//       schoolName: 'Northern Alberta Institute of Technology (NAIT)'
//     },
//     {
//       id: 'THEUNIVERISTYOFTEXASATDALLAS',
//       schoolName: 'The Univeristy of Texas at Dallas'
//     },
//     { id: 'UNIVERSITYOFWYOMING', schoolName: 'University of Wyoming' },
//     {
//       id: 'CALSTATESANBERNARDINO',
//       schoolName: 'Cal state San Bernardino '
//     },
//     { id: 'ADELPHIUNIVERSITY', schoolName: 'Adelphi University ' },
//     {
//       id: 'UNIVERSITYOFCALIFORNIAIRVINE',
//       schoolName: 'University of California, Irvine'
//     },
//     {
//       id: 'THEUNIVERSITYOFTEXASATDALLAS',
//       schoolName: 'The University of Texas at Dallas '
//     },
//     { id: 'UNIVERSITYOFTORONTO', schoolName: 'University of Toronto ' },
//     {
//       id: 'UNIVERSITYOFMINNESOTATWINCITIES',
//       schoolName: 'University of Minnesota - Twin Cities'
//     },
//     {
//       id: 'UNIVERSITYOFILLINOISATCHICAGO',
//       schoolName: 'university of illinois at chicago'
//     },
//     {
//       id: 'CITYOFNEWYORKHUNTERCOLLEGE',
//       schoolName: 'City of New York- Hunter College'
//     },
//     {
//       id: 'FLORIDAINTERNATIONALUNIVERSITY',
//       schoolName: 'Florida International University'
//     },
//     {
//       id: 'NORTHWESTERNUNIVERSITY',
//       schoolName: 'Northwestern University '
//     },
//     { id: 'DUKE', schoolName: 'Duke' },
//     {
//       id: 'UNIVERSITYOFTEXASATAUSTIN',
//       schoolName: 'University of Texas at Austin '
//     },
//     { id: 'UNIVERSITYOFMICHIGAN', schoolName: 'University of Michigan' },
//     {
//       id: 'UNIVERSITYOFMICHIGANANNARBOR',
//       schoolName: 'University of Michigan Ann Arbor'
//     },
//     { id: 'SANTACLARAUNIVERSITY', schoolName: 'Santa Clara University' },
//     {
//       id: 'CALIFORNIASTATEPOLYTECHNICUNIVERSITYPOMONA',
//       schoolName: 'California state polytechnic university, Pomona'
//     },
//     { id: 'PURDUEUNIVERSITY', schoolName: 'Purdue University' },
//     {
//       id: 'UNIVERSITYOFCALIFORNIABERKELEY',
//       schoolName: 'University of California Berkeley'
//     },
//     {
//       id: 'WESTVALLEYCOMMUNITYCOLLEGE',
//       schoolName: 'West Valley Community College'
//     },
//     {
//       id: 'HERIOTWATTUNIVERSITY(EDINBURGH)',
//       schoolName: 'Heriot-Watt University (Edinburgh)'
//     },
//     {
//       id: 'UNIVERSITYOFWASHINGTONBOTHELL',
//       schoolName: 'University of Washington Bothell'
//     },
//     {
//       id: 'CENTRALMICHIGANUNIVERSITY',
//       schoolName: 'Central Michigan University '
//     },
//     {
//       id: 'UNIVERSITYOFCALIFORNIADAVIS',
//       schoolName: 'University of California, Davis'
//     },
//     { id: 'BROWARDCOLLEGE', schoolName: 'Broward College' }
//   ];
// const airtableSchools = {
//         "0":"San Jose State University",
//         "4":"University of California Riverside ",
//         "6":"Mission College",
//         "8":"Colorado State University",
//         "9":"University of South Florida",
//         "10":"University of California Davis",
//         "12":"univeristy of califronia santa cruz",
//         "16":"University of Michigan ",
//         "17":"Stanford University",
//         "19":"University of Houston - Main Campus",
//         "20":"Northwestern University",
//         "21":"University of Toronto",
//         "22":"Grand Valley State University ",
//         "23":"Rutgers University Newark",
//         "24":"Normandale community college ",
//         "25":"Northeastern University",
//         "26":"New Jersey Institute of Technology",
//         "27":"University of Illinois at Chicago ",
//         "28":"University of Washington",
//         "29":"University of Maryland, College Park",
//         "30":"University of California, Riverside",
//         "31":"Loyola University Chicago",
//         "32":"Denison University ",
//         "33":"Georgia institute of technology",
//         "34":"University of Texas at Dallas",
//         "35":"Northern Virginia Community College",
//         "36":"University of Miami",
//         "37":"University of Windsor",
//         "38":"Rutgers University - New Brunswick",
//         "39":"University of Texas Southwestern Medical Center",
//         "40":"Haas school of business ",
//         "42":"The George Washington University",
//         "43":"Baruch college",
//         "44":"University of California Berkeley",
//         "45":"Hunter College",
//         "47":"York University (Toronto, Canada)",
//         "48":"DePaul University ",
//         "49":"University of Florida",
//         "50":"National University of Singapore",
//         "52":"Brooklyn College",
//         "53":"University College London",
//         "54":"University at Albany, SUNY",
//         "55":"Santa Clara University ",
//         "57":"University of Cincinnati ",
//         "58":"Teaneck High School",
//         "59":"Rutgers University",
//         "60":"University of Western Ontario",
//         "61":"Arizona State University ",
//         "62":"University of Maryland",
//         "63":"University of Texas at Austin",
//         "64":"Georgia Institute of Technology",
//         "65":"Auburn University ",
//         "66":"university of missouri-kansas city ",
//         "67":"University of California Los Angeles",
//         "68":"The University of Texas at Dallas",
//         "71":"University of the Incarnate Word ",
//         "72":"George Mason University ",
//         "73":"University of Houston ",
//         "74":"University of California Berkeley",
//         "75":"University of Pennsylvania ",
//         "76":"University of Manchester",
//         "77":"University of Illinois at Chicago",
//         "78":"University of Houston",
//         "80":"Rowan University ",
//         "81":"Drexel University",
//         "82":"The London school of economics and political science",
//         "83":"Aston University",
//         "84":"Ryerson University",
//         "85":"Binghamton University ",
//         "86":"North Carolina State University",
//         "87":"Northern Alberta Institute of Technology (NAIT)",
//         "88":"The Univeristy of Texas at Dallas",
//         "91":"University of Wyoming",
//         "92":"Cal state San Bernardino ",
//         "93":"Adelphi University ",
//         "94":"University of California, Irvine",
//         "96":"The University of Texas at Dallas ",
//         "97":"University of Toronto ",
//         "99":"University of Minnesota - Twin Cities",
//         "100":"university of illinois at chicago",
//         "101":"City of New York- Hunter College",
//         "102":"Florida International University",
//         "106":"Northwestern University ",
//         "109":"Duke",
//         "111":"University of Texas at Austin ",
//         "112":"University of Michigan",
//         "113":"University of Michigan Ann Arbor",
//         "118":"Santa Clara University",
//         "119":"California state polytechnic university, Pomona",
//         "120":"Purdue University",
//         "124":"University of California Berkeley",
//         "126":"West Valley Community College",
//         "129":"Heriot-Watt University (Edinburgh)",
//         "133":"University of Washington Bothell",
//         "134":"Central Michigan University ",
//         "136":"University of California, Davis",
//         "137":"Broward College"
// };

const uniqueSchools = (schools)=>{
    let schoolsArray = Object.keys(schools).map((key)=>{
        return {id: schools[key].replace(/\s+/g, "").replace(/([.,-])/g, "").toUpperCase(), schoolName: schools[key]} 
    });
    // let uniqueSet = schoolsArray.filter((schoolObj1, schoolObj2) =>{
    //     //console.log(schoolObj1);
    //     return schoolObj1["id"] != schoolObj2["id"]
    // })
    //let uniqueSet = [...new Set(schoolsArray))];

    // let uniqueSetObj = uniqueSet.map((schoolID)=>{
    //     return {
    //             id: schoolID, 
    //         }
    // })
    //console.log("spaces and char removed length", schoolsArray.length);
    // console.log("Unique set", uniqueSet, "Length", uniqueSet.length);
    //console.log("unique set object", uniqueSetObj, "len of unique set obj", uniqueSetObj.length)
    return schoolsArray;
}
const removeDuplicates = (notUniqueSchools) => {
    let schoolIDs = [];
    let nonDuplSchools = [];
    notUniqueSchools.map((schoolObj)=>{
        if(!schoolIDs.includes(schoolObj.id)){
            nonDuplSchools.push(schoolObj);
            schoolIDs.push(schoolObj.id);
        }
    })
    return nonDuplSchools;
}
// let notUniqueSchools = uniqueSchools(airtableSchools);
// let nonDupSchool = removeDuplicates(notUniqueSchools);
// console.log(nonDupSchool);



module.exports = tureUniqueSchools;
